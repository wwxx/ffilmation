		  ...
		  */
			public function getXML():XMLList {
				...
				for(var j:Number=0;j<this.result.length;j++) {
					...
					ret+=new XML('<object orientation="'+orientation+'" id="Scatter'+this.id+'_Obj'+j+'" definition="'+this.result[j].def+'" x="'+this.result[j].x+'" y="'+this.result[j].y+'" z="'+this.result[j].z+'"/>')
				}
				return ret
			}
