// Character class
package org.ffilmation.engine.logicSolvers.coverageSolver {
		...
		public class fCoverageSolver {
			...
			*/
			public static function calculateCoverage(from:fRenderableElement,to:*,x:Number,y:Number,z:Number):int {
				...
				try {
					...
					if(from is fFloor) {
						if(to is fFloor) return fCoverageSolver.calculateCoverageFloorFloor(from,to as fFloor,x,y,z)
						if(to is fWall) {
							var w:fWall = to as fWall
							if(w.vertical) return fCoverageSolver.calculateCoverageFloorVerticalWall(from,w,x,y,z)
							else return fCoverageSolver.calculateCoverageFloorHorizontalWall(from,w,x,y,z)
						}
						if(to is fObject) return fCoverageSolver.calculateCoverageFloorObject(from,to as fObject,x,y,z)
					}
					if(from is fWall) {
						if(to is fFloor) return fCoverageSolver.calculateCoverageWallFloor(from,to as fFloor,x,y,z)
						if(to is fWall) {
							w = to as fWall
							if(w.vertical) return fCoverageSolver.calculateCoverageWallVerticalWall(from,w,x,y,z)
							else return fCoverageSolver.calculateCoverageWallHorizontalWall(from,w,x,y,z)
						}
						if(to is fObject) return fCoverageSolver.calculateCoverageWallObject(from,to as fObject,x,y,z)
					}
					if(from is fObject) {
						if(to is fFloor) return fCoverageSolver.calculateCoverageObjectFloor(from as fObject,to as fFloor,x,y,z)
						if(to is fWall) {
							w = to as fWall
							if(w.vertical) return fCoverageSolver.calculateCoverageObjectVerticalWall(from as fObject,w,x,y,z)
							else return fCoverageSolver.calculateCoverageObjectHorizontalWall(from as fObject,w,x,y,z)
						}
						if(to is fObject) return fCoverageSolver.calculateCoverageObjectObject(from as fObject,to as fObject,x,y,z)
					}
			...
			*/
			private static function calculateCoverageFloorFloor(from:Object,to:fFloor,x:Number,y:Number,z:Number):int {
			   ...
			   if(from.z>to.z && from.z<z) {
			      ...
			      if(pLeft>(to.x+to.width)) return fCoverage.NOT_SHADOWED
			
			      var pUp:Number = y+(from.y-y)*dz
			   		if(pUp>(to.y+to.depth)) return fCoverage.NOT_SHADOWED
			
			      var pDown:Number = y+(from.y+from.depth-y)*dz
			   		if(pDown<to.y) return fCoverage.NOT_SHADOWED
			
			      var pRight:Number = x+(from.x+from.width-x)*dz
					  if(pRight<to.x) return fCoverage.NOT_SHADOWED
			      ...
			      if(pUp<=to.y && pDown>=(to.y+to.depth) && pLeft<=to.x && pRight>=(to.x+to.width)) {
			      	  ...
			      	  if(from is fFloor) {
			      	  	...
			      	  	for(var h:int=0;h<len;h++) {
			      	  		if(fCoverageSolver.calculateCoverageFloorFloor(from.holes[h].bounds,to,x,y,z)!=fCoverage.NOT_SHADOWED) return fCoverage.SHADOWED
			      	    }
			      	  }
			...
			*/
			private static function calculateCoverageWallFloor(from:Object,to:fFloor,x:Number,y:Number,z:Number):int {
			   ...
			   if(from.top<=to.z || from.z>=z) return fCoverage.NOT_SHADOWED
			   ...
			   if(from.vertical) {
			      ...
			      if(from.x<=x) {
			         ...
			         if(from.y1<to.y) {
							 ...
							 if(from.z>to.z) {
								  ...
								  if(pRight>(to.x+to.width)) return fCoverage.NOT_SHADOWED
							 }
			      ...
			      if(from.y<y) {
			         ...
			         if(from.top<z) {
			   				 dz = 1+(from.top-to.z)/(z-from.top)
				         pUp = 1+y+(from.y-y)*dz
			  	       if(pUp<to.y) return fCoverage.NOT_SHADOWED
							 }
			         ...
			         if(from.x1<to.x) {
							 ...
							 if(from.z>to.z) {
						 	 		dzb = 1+(from.z-to.z)/(z-from.z)
									pDown = -1+y+(from.y-y)*dzb
								  if(pDown>(to.y+to.depth)) return fCoverage.NOT_SHADOWED
							 }
			...
			*/
			private static function calculateCoverageObjectFloor(from:fObject,to:fFloor,x:Number,y:Number,z:Number):int {
			   ...
			   if(from.y>(to.y+to.depth) && y<from.y) return fCoverage.NOT_SHADOWED
			   if(from.x<to.x && x>from.x) return fCoverage.NOT_SHADOWED
			   	 ...
			   	 if((to.y<pUp && from.y>=to.y) || (to.y>pDown && (from.y+from.depth)<=to.y) || (to.x0>pRight && (from.x+from.width)<=to.x0) || (to.x1<pLeft && from.x>=to.x1)) {
				       ...
				       return fCoverage.NOT_SHADOWED
			       	  ...
			       	  if(from is fFloor) {
			       	  	len = from.holes.length
			       	  	for(var h:int=0;h<len;h++) {
			       	  		if(fCoverageSolver.calculateCoverageFloorHorizontalWall(from.holes[h].bounds,to,x,y,z)!=fCoverage.NOT_SHADOWED) return fCoverage.SHADOWED
			       	  	}
			       	  }
					 			return fCoverage.COVERED
					 		}
			      ...
			      if((to.y0>pDown && (from.y+from.depth)<=to.y0) || (to.y1<pUp && from.y>=to.y1) || (to.x>pRight && (from.x+from.width)<=to.x) || (to.x<pLeft && from.x>=to.x)) {
			         ...
			         return fCoverage.NOT_SHADOWED
			      	  ...
			      	  if(from is fFloor) {
			      	  	len = from.holes.length
			      	  	for(var h:int=0;h<len;h++) {
			      	  		if(fCoverageSolver.calculateCoverageFloorVerticalWall(from.holes[h].bounds,to,x,y,z)!=fCoverage.NOT_SHADOWED) return fCoverage.SHADOWED
			      	  	}
			      	  }
